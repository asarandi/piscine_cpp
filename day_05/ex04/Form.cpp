...
/*   Updated: 2018/01/16 17:25:38 by asarandi         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void		Form::beSigned(Bureaucrat bur)
{
	...
	std::cout << " (grade " << bur.getGrade() << ") signs a ";
	std::cout << this->_name;
	std::cout << " (s.grade " << this->_gradeToSign << ", ex.grade " << this->_gradeToExec << ")";
	std::cout << " targeted on " << this->_target;
	if (this->_signed == true)
...
void	Form::execute(Bureaucrat const & executor) const
{
	if (this->_signed != true)
	{
		std::cout << "Form [" << this->_name << "] can not be executed because it is not signed!" << std::endl;
		return ;
	}
	if (executor.getGrade() > this->_gradeToExec)
	{
		std::cout <<  "Form [" << this->_name << "] can not be executed because the bureaucrat's grade is too low: [" << executor.getName() << "], [" << executor.getGrade() << "]" << std::endl;
		std::cout <<  "Throwing Exception Right Meow!" << std::endl;
		throw Form::GradeTooLowException();
		return ;
	}
	std::cout << "Bureaucrat ";
	std::cout << executor.getName();
	std::cout << " (grade " << executor.getGrade() << ") executes a ";
	std::cout << this->_name;
	std::cout << " (s.grade " << this->_gradeToSign << ", ex.grade " << this->_gradeToExec << ")";
	std::cout << " targeted on " << this->_target;
	if (this->_signed == true)
